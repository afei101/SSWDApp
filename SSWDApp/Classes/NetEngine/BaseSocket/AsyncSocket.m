...
- (BOOL)attachStreamsToRunLoop:(NSRunLoop *)runLoop error:(NSError **)errPtr
{
    //NSLog(@"attachStreamsToRunLoop");
	// Get the CFRunLoop to which the socket should be attached.
    //NSRunLoop *runloop1 = [NSRunLoop currentRunLoop];
    //CFRunLoopRef therunloop1 = [runloop1 getCFRunLoop];
	theRunLoop = (runLoop == nil) ? [[NSRunLoop currentRunLoop] getCFRunLoop] : [runLoop getCFRunLoop];
...
- (void)doStreamOpen
{
    //NSLog(@"AsyncSocket doStreamOpen");
	if ((theFlags & kDidCompleteOpenForRead) && (theFlags & kDidCompleteOpenForWrite))
	{
        ...
        //NSLog(@"doStream open maybeDequeueRead");
...
- (void)maybeDequeueRead
{
	...
	if((theCurrentRead == nil) && (theReadStream != NULL))
	{
		if([theReadQueue count] > 0)
		{
            ...
            //NSLog(@"maybeDequeueRead 1");
			[theReadQueue removeObjectAtIndex:0];
            //NSLog(@"maybeDequeueRead 2");
...
- (void)completeCurrentRead
{
	...
	//NSLog(@"AsynSocket completeCurrentRead");
	NSData *result;
...
- (void)doSendBytes
{
	...
	while (!done && !error && [self canAcceptBytes])
	{
        ...
        //NSLog(@"%d" , [theCurrentWrite->buffer length]);
